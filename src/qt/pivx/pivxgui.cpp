...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Epsium client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Epsium"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Epsium"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
