...
const char * const PIVX_CONF_FILENAME = "epsium.conf";
const char * const PIVX_PID_FILENAME = "epsium.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\epsium
// Windows >= Vista: C:\Users\Username\AppData\Roaming\epsium
// Mac: ~/Library/Application Support/epsium
// Unix: ~/.epsium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Epsium";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\epsiumParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\epsiumParams
    // Mac: ~/Library/Application Support/epsiumParams
    // Unix: ~/.epsium-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EpsiumParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "EpsiumParams";
#else
    // Unix
    return pathRet / ".epsium-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty epsium.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Epsium config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override epsium.conf
