...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected EPS and zEPS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zEPSsupply']:
                assert_equal(node['zEPSsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check EPS and zEPS supply at the beginning
        # ------------------------------------------
        # zEPS supply: 2 coins for each denomination
        ...
        # EPS supply: block rewards minus burned fees for minting
        ...
        # Verify that EPS and zEPS supplies were properly updated after the spends and reorgs
        self.log.info("Check EPS and zEPS supply...")
